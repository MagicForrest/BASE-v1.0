...
# read the FRY data.table
FRY_dt <- fread(file.path(base_dir, "FRY_v2.0", paste0("FIRECCI51_6D_NCV", ".txt")))

# set names to match the master DT and drop some others
FRY_dt[ , Lon := LON_GRID]
FRY_dt[ , Lat := LAT_GRID]
FRY_dt[ , Year := YR]
FRY_dt[ , Month := MONTH]
...
FRY_dt[ , MONTH := NULL]
FRY_dt[ , DAY := NULL]
FRY_dt[ , LON_GRID := NULL]
FRY_dt[ , LON_INDEX := NULL]
FRY_dt[ , LON := NULL]

FRY_dt[ , LAT_GRID := NULL]
FRY_dt[ , LAT_INDEX := NULL]
FRY_dt[ , LAT := NULL]

# 
master_full_dt <- merge.data.table(master_full_dt, FRY_dt, by = c("Lon", "Lat", "Year", "Month"), all.x = FALSE, all.y = TRUE)

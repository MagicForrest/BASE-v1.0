...
add_long_term_means_and_deviations <- function(dt, gs_dt){
  
  # calculate annual values (all months, JJA and growing season)
  annual_means_dt <- dt[, lapply(.SD, FUN=sum, na.rm = TRUE), by = c("Lon", "Lat", "Year"), .SDcols = c("value")]
  setnames(annual_means_dt, "value", "annual_value")
  JJA_means_dt <- dt[ Month %in% c(6,7,8) , lapply(.SD, FUN=sum, na.rm = TRUE), by = c("Lon", "Lat", "Year"), .SDcols = c("value")]
  setnames(JJA_means_dt, "value", "JJA_value")
  # do growing season means
  gs_subset_dt <- merge.data.table(dt, gs_dt, by =c("Lon", "Lat", "Month"))
  gs_means_dt <- gs_subset_dt[, lapply(.SD, FUN=sum, na.rm = TRUE), by = c("Lon", "Lat", "Year"), .SDcols = c("value")]
  print(gs_means_dt)
...
add_antecedent_values <- function(dt){
  ...
  dt[, maxvalue12 := data.table::shift(data.table::frollapply(value, FUN = max, align = "center", n = 12)), by=c("Lon", "Lat")]

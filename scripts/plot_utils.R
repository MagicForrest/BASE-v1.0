...
nOptCols <- function(x, rows.more.than.cols = 0, type = "Spatial", nlayers, ntimes) {
  ...
  #  a single field (assume plotting all layers and Days/Months/Seasons/Years)
    ...
    for(this.x in x) {
      ...
      if(tolower(type) == "spatial") {
        if(missing(nlayers)) nlayers <- length(layers(this.x))
        if(missing(ntimes)) {
          ntimes <- 1
          if("Day" %in% getDimInfo(this.x)) ntimes <- ntimes * getDimInfo(this.x, "size")$Day
          else if("Month" %in% getDimInfo(this.x)) ntimes <- ntimes * getDimInfo(this.x, "size")$Month
          else if("Season" %in% getDimInfo(this.x)) ntimes <- ntimes * getDimInfo(this.x, "size")$Season
          if("Year" %in% getDimInfo(this.x)) ntimes <- ntimes * getDimInfo(this.x, "size")$Year
        }

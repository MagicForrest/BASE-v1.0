...
here::i_am("scripts/tests/plot_BA_patterns_high_lats.R")
library(here)
...
ba_cuts <- c(0, 0.1, 0.2, 0.5, 1, 2 ,5,10,20,50,100,200,500,1000,2000,5000,10000)
...
# select only high lats
cropland_dt <- cropland_dt[ Lat > 55 & Lon > 4, ] 
ncv_dt <- ncv_dt[ Lat > 55  & Lon > 4, ] 
...
magicPlot(p = spatial_plot, filename = file.path(plot_dir, "High_lats_04_Spatial_BA"), width = 1100, height = 1200, type = "png")

pdf(file = file.path(plot_dir, paste0("High_lats_04_Spatial_BA.pdf")), width = 11, height = 12)
print(spatial_plot)
dev.off()

pdf(file = file.path(plot_dir, paste0("fig_04.pdf")), width = 11, height = 12)
print(spatial_plot)
dev.off()
...
print(iav_plot)
magicPlot(p = iav_plot, filename = file.path(plot_dir, "High_lats_05_IAV_BA"), width = 1200, height = 1000)

plot_dim_units = "px"
pdf(file = file.path(plot_dir, paste0("High_lats_05_IAV_BA.pdf")), width = 12, height = 10)
print(iav_plot)
dev.off()

pdf(file = file.path(plot_dir, paste0("fig_05.pdf")), width = 12, height = 10)
print(iav_plot)
dev.off()
...
print(seasonal_plot)
magicPlot(p = seasonal_plot, filename = file.path(plot_dir, "High_lats_06_Seasonal_BA"), width = 1200, height = 1000)

pdf(file = file.path(plot_dir, paste0("High_lats_06_Seasonal_BA.pdf")), width = 12, height = 10)
print(seasonal_plot)
dev.off()

pdf(file = file.path(plot_dir, paste0("fig_06.pdf")), width = 12, height = 10)
print(seasonal_plot)
dev.off()

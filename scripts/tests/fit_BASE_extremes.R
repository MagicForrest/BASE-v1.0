...
# version
fit_batch_version <- "BASE_v1.0_low_years"
...
high_years <- c(2003, 2007, 2013)
low_years <- c(2006, 2010, 2014)
...
source(here("scripts/extreme_sensitivity_models.R"))
...
for(model_spec in model_specifications_list) {
  ...
  if(nrow(master_dt) > MIN_DATA_POINTS) {
    ...
    fitting_gridcells <- valid_subset_dt
    #training_dt <- valid_subset_dt[ Year %in% high_years, ]
    training_dt <-subset(valid_subset_dt, Year %in% low_years)
    print(unique(training_dt[["Year"]]))
    
    
    #testing_dt <- valid_subset_dt[ Year %in% low_years, ]
    testing_dt <-subset(valid_subset_dt, Year %in% high_years)
    print(unique(testing_dt[["Year"]]))
    ...
    if(do_fit) {
      ...
      print(" ** Calculating gridcell area and burnt area for training data")
      tic()
      ...
      predicted_training_dt[, Predicted_burnt_fraction := Predicted_burnt_fraction * 100]
      predicted_training_dt[, Observed_burnt_fraction := Observed_burnt_fraction * 100]
      toc()
      
      print(" ** Calculating gridcell area and burnt area for testing data")
      tic()
      ...
      predicted_testing_dt[, Predicted_burnt_fraction := Predicted_burnt_fraction * 100]
      predicted_testing_dt[, Observed_burnt_fraction := Observed_burnt_fraction * 100]
      toc()
      ...
      print(testing_NME)
      print(unscaled_NME)
      summary_metrics[["Full NME"]] <- round(unscaled_NME, 3)
      summary_metrics[["Training NME"]] <- round(training_NME, 3)
      summary_metrics[["Testing NME"]] <- round(testing_NME, 3)
    ...
    tic()
    print(3)
    template_dt <- calculatePredictionDT(dt = valid_subset_dt, model = this_m, npoints = 100) 
    print(4)
    toc()
    ...
    for(this_var in c(linear_terms, quadratic_terms, smooth_terms)){
      ...
      effect_plot <- effect_plot + labs(caption = caption_text, y = paste("Burnt Fraction [-] (other covariates fixed at mean)"))
    ...
    for(this_interaction in interaction_terms){
      ...
      if(length(these_vars) == 2) {
        ...
        these_ranges[[these_vars[2]]] <- range(master_dt[[these_vars[2]]], na.rm = TRUE)
        interaction_plot <- plotInteractionTerm(vars = these_vars, model =  this_m, dt = template_dt, ranges = these_ranges)
      }
      ...
      magicPlot(p = interaction_plot, filename = file.path(plot.dir, paste("Predictor_Interaction", paste0(these_vars[1], "x", these_vars[2]), sep = "_")), height = 1300, width =1800)
    }
    ...
    for(scaled_or_unscaled in c("GLM", "Scaled")) {
      ...
      if(scaled_or_unscaled == "GLM" | (scaled_or_unscaled == "Scaled" & apply_scaling)) {
        ...
        summary_metrics[[paste(scaled_or_unscaled, "Spatial NME")]] <- round(spatial_stats$NME, 3)
        summary_metrics[[paste("Null", scaled_or_unscaled, "Spatial NME")]] <- round(spatial_stats$`null NME`, 3)
        print(paste("Final spatial burnt area NME =",  round(spatial_stats$NME, 3)))
        ...
        summary_metrics[[paste(scaled_or_unscaled, "Null MPD")]] <- round(seasonal_stats$null_MPD, 3)
        summary_metrics[[paste(scaled_or_unscaled, "MPD")]] <- round(seasonal_stats$MPD, 3)
        summary_metrics[[paste(scaled_or_unscaled, "NULL NME_conc")]] <- round(seasonal_stats$null_NME_con, 3)
        summary_metrics[[paste(scaled_or_unscaled, "NME_conc")]] <- round(seasonal_stats$NME_conc, 3)
        toc()
        
        tic()
        seasonal_stats <- doSeasonalStats(predicted_dt, layers1 = "Predicted_burnt_area_raw", layers2 = "Observed_burnt_area")
        summary_metrics[[paste(scaled_or_unscaled, "Full Null MPD")]] <- round(seasonal_stats$null_MPD, 3)
        summary_metrics[[paste(scaled_or_unscaled, "Full MPD")]] <- round(seasonal_stats$MPD, 3)
        summary_metrics[[paste(scaled_or_unscaled, "Full NULL NME_conc")]] <- round(seasonal_stats$null_NME_con, 3)
        summary_metrics[[paste(scaled_or_unscaled, "Full NME_conc")]] <- round(seasonal_stats$NME_conc, 3)
        toc()
        ...
        iav_stats <- continuousComparison(x = predicted_dt_yearly, 
                                          ...
                                          layers2 = "Observed_burnt_area", 
                                          area = TRUE, additional = NULL)
        summary_metrics[[paste(scaled_or_unscaled, "IAV NME")]] <- round(iav_stats$NME, 3)
        summary_metrics[[paste("Null", scaled_or_unscaled, "IAV NME")]] <- round(iav_stats$`null NME`, 3)
        print(paste("Final IAV burnt area NME =",  round(iav_stats$NME, 3)))
